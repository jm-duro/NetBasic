...
constant BLOCK_SIZE = 4096, IFF_PROMISC = #100
...
public type eunet_is_inetaddr(object s)
...
public function eunet_get_errormode()
  return error_mode
end function

-------------------------------------------------------------------------------

public function eunet_set_errormode(integer mode)
...
public function eunet_get_error()
...
public function eunet_get_blocksize()
  return BLOCK_SIZE
end function

-------------------------------------------------------------------------------

public function eunet_delay(atom millisec)
...
global function eunet_get_promiscuous_mode(atom sockid, sequence itf)
  ...
  free(ifreq)
  if and_bits(flags, IFF_PROMISC) then
    return 1
  else
    return 0
  end if
...
public function eunet_get_iface_list()
  ifdef WINDOWS then
...
public function eunet_get_iface_details(sequence iface_name)
  ifdef WINDOWS then
...
public function eunet_bind(atom socket, sequence inet_addr)
  ifdef WINDOWS then
...
public function eunet_listen(atom socket, integer pending_conn_len)
  ifdef WINDOWS then 
...
public function eunet_accept(atom socket)
  ifdef WINDOWS then 
...
public function eunet_connect(atom socket, sequence inet_addr)
  ifdef WINDOWS then
...
public function eunet_new_socket(integer family, integer sock_type, integer protocol)
...
public function eunet_close_socket(atom socket)
  ifdef WINDOWS then
...
public function eunet_shutdown_socket(atom socket, atom method)
  ifdef WINDOWS then
...
public function eunet_poll(sequence socket_list, atom timeout)
  ifdef WINDOWS then
...
public function eunet_send(atom socket, sequence data, atom flags)
  ifdef WINDOWS then
...
public function eunet_sendto(atom socket, sequence data, atom flags, sequence inet_addr)
  ifdef WINDOWS then
...
public function eunet_recv(atom socket, atom flags)
  ifdef WINDOWS then
...
public function eunet_recvfrom(atom socket, atom flags)
  ifdef WINDOWS then
...
public function eunet_get_socket_options(atom socket, integer level, integer optname)
  ifdef WINDOWS then
...
public function eunet_set_socket_options(atom socket, integer level, integer optname, object val)
  ifdef WINDOWS then
...
public function eunet_dnsquery(sequence dname, integer q_type, atom options)
  ifdef WINDOWS then
...
public function eunet_getmxrr(sequence dname, atom options)
  object rtn
...
public function eunet_getnsrr(sequence dname, atom options)
...
public function eunet_gethostbyname(sequence name)
  ifdef WINDOWS then
...
public function eunet_getaddrinfo(object node, object service, object hints)
  ifdef WINDOWS then
...
public function eunet_get_addrinfo(object node, object service, object hints)
...
public function eunet_urlencode(sequence what)
  ...
  for loop0 = 1 to length(what) do
    if find(what[loop0],alphanum) then
      encoded &= what[loop0]
    else 
      junk = what[loop0]
      junk1 = floor(junk / 16)
...
public function eunet_get_sendheader(object field)
...
-- we'll return a {"Name","spacer","value"} format
  ...
  if sequence(field) then
    ...
    for loop0 = 1 to length(sendheader) do
      if equal(upperfield,upper(sendheader[loop0][1])) then
        return sendheader[loop0]
      end if
    end for
    return -1
...
public procedure eunet_set_sendheader_default()
  -- sets some defaults
...
public procedure eunet_set_sendheader(object whatheader, sequence whatdata)
  ...
  -- Function added by Kathy Smith (Kat)(KAT12@coosahs.net), version 1.3.0
  for loop0 = 1 to length(sendheader) do
    if match(upper(whatheader),upper(sendheader[loop0][1])) then
      sendheader[loop0][3] = whatdata
      return
    end if
  end for
...
public procedure eunet_set_sendheader_useragent_msie()
...
function eunet_format_sendheader()
  ...
  httpversion = ""
  for loop0 = 1 to length(sendheader) do
    if not equal("",sendheader[loop0][3]) and 
       not equal("httpversion",sendheader[loop0][1]) and 
       not equal("POSTDATA",sendheader[loop0][1]) then
          if equal("GET",sendheader[loop0][1])
            then tempheader &= sendheader[loop0][1] & sendheader[loop0][2] & sendheader[loop0][3] & " " & httpversion & "\n"
            else tempheader &= sendheader[loop0][1] & sendheader[loop0][2] & sendheader[loop0][3] & "\r\n"
          end if  
    end if
    if equal("POSTDATA",sendheader[loop0][1]) and not equal("",sendheader[loop0][3]) then
      --temppostdata = urlencode(sendheader[loop0][3])
      temppostdata = sendheader[loop0][3]
      eunet_set_sendheader("Content-Length",sprintf("%d",length(temppostdata)))
    end if
    if equal("httpversion",sendheader[loop0][1]) and not equal("",sendheader[loop0][3]) then
      httpversion = sendheader[loop0][3]
    end if  
  end for
...
public function eunet_parse_httpheader(sequence header)
  ...
  while length(header) > 0 and cpos > 0 do
    parsedheader = append(parsedheader,{trim_(header[1..cpos-1])})
    header = header[cpos+1..length(header)]
    cpos = find('\n',header)
  end while
  if length(header) > 0 then
    parsedheader = append(parsedheader,{trim_(header)})
  end if
...
public procedure eunet_parse_recvheader(sequence header)
  ...
  for loop0 = 1 to length(recvheader) do
    place = match(": ",recvheader[loop0])
    if place then
      junk[1] = recvheader[loop0][1..place-1]
      junk[2] = recvheader[loop0][place+2..length(recvheader[loop0])]
      recvheader[loop0] = junk
    else
      if match("HTTP/",upper(recvheader[loop0])) then
        recvheader[loop0] = {"httpversion",recvheader[loop0]} -- what else to call that line?
      end if
    end if
  end for
end procedure
...
public function eunet_get_recvheader(object field)
sequence upperfield
  ...
  upperfield = upper(field)  
  for loop0 = 1 to length(recvheader) do
    if equal(upperfield,upper(recvheader[loop0][1])) then
      return recvheader[loop0] -- {"header_name","value"}
    end if
  end for
...
public function eunet_get_http(sequence inet_addr, sequence hostname, sequence file)
...
public function eunet_get_http_use_cookie(sequence inet_addr, sequence hostname,
                sequence file)
...
public function eunet_get_url(sequence url)
...
-------------------------------------------------------------------------------
-- WSA Cleanup (Windows only)
-------------------------------------------------------------------------------

public procedure wsa_cleanup()
  object void

  ifdef WINDOWS then
    void = c_func(wsacleanup_,{})
  end ifdef
end procedure
...
procedure test()
  ...
  str = eunet_get_url("http://www.yahoo.com")
  puts(1,str[2])
